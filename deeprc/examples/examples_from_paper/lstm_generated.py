# -*- coding: utf-8 -*-
"""
Simple example for training binary DeepRC classifier on datasets of category "LSTM-generated data"
...
import numpy as np
import torch
from deeprc.predefined_datasets import lstm_generated_dataset
from deeprc.architectures import DeepRC, SequenceEmbeddingCNN, AttentionNetwork, OutputNetwork
from deeprc.training import train, evaluate
parser = argparse.ArgumentParser()
parser.add_argument('id', help='ID of dataset. 0: "motif freq. 10%%", 1: "motif freq. 1%%", '
                               '2: "motif freq. 0.5%%", 3: "motif freq. 0.1%%", 4: "motif freq. 0.05%%"', type=int)
...
task_definition, trainingset, trainingset_eval, validationset_eval, testset_eval = lstm_generated_dataset(dataset_id=args.id)
...
train(model, task_definition=task_definition, trainingset_dataloader=trainingset,
      ...
      early_stopping_target_id='Known CMV status',  # Get model that performs best for this task
      validationset_eval_dataloader=validationset_eval, n_updates=args.n_updates, evaluate_at=args.evaluate_at,
      device=device, results_directory=f"results/lstm_generated_{args.id}"  # Here our results and trained models will be stored
      )
