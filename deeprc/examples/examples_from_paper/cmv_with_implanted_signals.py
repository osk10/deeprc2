...
import numpy as np
import torch
from deeprc.predefined_datasets import cmv_implanted_dataset
...
task_definition, trainingset, trainingset_eval, validationset_eval, testset_eval = cmv_implanted_dataset(dataset_id=args.id)
...
train(model, task_definition=task_definition, trainingset_dataloader=trainingset,
      ...
      early_stopping_target_id='status',  # Get model that performs best for this task
      validationset_eval_dataloader=validationset_eval, n_updates=args.n_updates, evaluate_at=args.evaluate_at,
      device=device, results_directory=f"results/cmv_with_implanted_signals_{args.id}"  # Here our results and trained models will be stored
      )
